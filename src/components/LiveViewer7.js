...
function LiveViewer7() {
  ...
  const initializeMeetingSession = useCallback(async (meetingData, attendeeData) => {
    ...
    session.audioVideo.realtimeSubscribeToVolumeIndicator(
      (attendeeId, volume, muted, signalStrength) => {
        ...
        if (volume > 0) {
          alert(`xVolume>0 ${Math.round(volume * 100)}% from ${attendeeId.substring(0, 8)}...`, { muted, signalStrength });
        } else {
          alert(`yVolume ${Math.round(volume * 100)}% from ${attendeeId.substring(0, 8)}...`, { muted, signalStrength });
        }
      }
    );
    ...
    if (session.audioVideo) {
      ...
      //   (attendeeId, isActiveSpeaker) => {
      //     if (isActiveSpeaker) {
      //       alert(`Active speaker: ${attendeeId}`);
      //     } else {
      //       alert(`Inactive speaker: ${attendeeId}`);
      //     }
      //   }
      // );
    }
  ...
  //         alert(`xVolume>0 ${Math.round(volume * 100)}% from ${attendeeId.substring(0, 8)}...`, { muted, signalStrength });
  //       } else {
  //         console.log(`yVolume ${Math.round(volume * 100)}% from ${attendeeId.substring(0, 8)}...`, { muted, signalStrength });
  //       }
  //     }
  //   );
  ...
  //     (attendeeId, isActiveSpeaker) => {
  //       if (isActiveSpeaker) {
  //         console.log(`Active speaker: ${attendeeId}`);
  //       } else {
  //         console.log(`Inactive speaker: ${attendeeId}`);
  //       }
  //     }
  //   );
